<template>
 <div
  ...
  >
    ...
    <div class="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-10 relative z-10">
      <!-- SUCCESS STATE -->
      <Transition
        ...
        <div
          ...
          <div class="relative">
            ...
            <div class="relative w-20 h-20 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full flex items-center justify-center shadow-[0_8px_32px_rgba(16,185,129,0.4)] animate-success-pop">
          ...
          <div class="flex flex-col items-center gap-2 text-center max-w-md">
            <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-800 dark:text-slate-100">
          ...
          <!-- Summary Card -->
          <div class="w-full max-w-md bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm border border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-6 shadow-lg">
            <div class="flex flex-col gap-3">
              ...
              <div class="flex justify-between items-start text-sm gap-2">
                ...
                <span class="font-semibold text-slate-700 dark:text-slate-300 text-right line-clamp-2 max-w-[240px]">{{ submittedData?.tagline }}</span>
              </div>
              <div class="h-px bg-slate-100 dark:bg-slate-800/50" />
              <div class="flex justify-between items-start text-sm gap-2">
                <span class="text-slate-400 dark:text-slate-500 shrink-0">แนะนำตัว</span>
                <span class="font-semibold text-slate-600 dark:text-slate-400 text-right line-clamp-2 max-w-[240px]">{{ submittedData?.bio }}</span>
              </div>
              <div class="h-px bg-slate-100 dark:bg-slate-800/50" />
          ...
          <Button
            ...
            icon-pos="right"
            pt:root:class="bg-gradient-primary border-none shadow-md hover:shadow-lg hover:scale-[1.02] transition-all px-8 py-3 rounded-2xl cursor-pointer"
      ...
      <div v-if="!isSubmitted">
        <!-- Header Section — matches index.vue pattern -->
        <div class="flex flex-col gap-2 mb-6">
          <div class="flex items-center gap-3 mb-1">
            <ButtonBack
              ...
              label="ย้อนกลับ"
              class="text-sm font-semibold text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition" />
          </div>
        ...
        <!-- Stepper Form Card -->
        <div class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 sm:p-8 shadow-sm transition-all duration-300">
          <StepperRent
            v-model="activeStep"
